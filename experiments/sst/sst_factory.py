...
def get_loss_weight_symbol(data, mask, seq_len):
    if cfg.MODEL.USE_BALANCED_LOSS:
        balancing_weights = cfg.SST.EVALUATION.BALANCING_WEIGHTS
        weights = mx.sym.ones_like(data) * balancing_weights[0]
        thresholds = [rainfall_to_pixel(ele) for ele in cfg.SST.EVALUATION.THRESHOLDS]
    ...
    if cfg.MODEL.TEMPORAL_WEIGHT_TYPE == "same":
        ...
        assert upper >= 1.0
        temporal_mult = 1 + mx.sym.arange(start=0, stop=seq_len) * (upper - 1.0) / (
            seq_len - 1.0
        )
...
class SSTNowcastingFactory(EncoderForecasterBaseFactory):
    def __init__(
        self, batch_size, in_seq_len, out_seq_len, ctx_num=1, name="sst_nowcasting"
    ):
        super(SSTNowcastingFactory, self).__init__(
        ...
        self._central_region = cfg.SST.EVALUATION.CENTRAL_REGION
    ...
    def loss_sym(
        ...
        if cfg.MODEL.L2_LAMBDA > 0:
            avg_mse = mx.sym.MakeLoss(
                avg_mse, grad_scale=global_grad_scale * cfg.MODEL.L2_LAMBDA, name="mse"
            )
        else:
            avg_mse = mx.sym.BlockGrad(avg_mse, name="mse")
        if cfg.MODEL.L1_LAMBDA > 0:
            avg_mae = mx.sym.MakeLoss(
                avg_mae, grad_scale=global_grad_scale * cfg.MODEL.L1_LAMBDA, name="mae"
            )
        else:
            avg_mae = mx.sym.BlockGrad(avg_mae, name="mae")
        if cfg.MODEL.GDL_LAMBDA > 0:
            avg_gdl = mx.sym.MakeLoss(
                avg_gdl, grad_scale=global_grad_scale * cfg.MODEL.GDL_LAMBDA, name="gdl"
            )
        else:
