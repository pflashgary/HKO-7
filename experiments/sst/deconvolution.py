...
from nowcasting.sst_benchmark import SSTBenchmarkEnv
...
# Arguments
def argument_parser():
    parser = argparse.ArgumentParser(description="Deconvolution baseline for SST")
...
def dataset_args(parser):
    group = parser.add_argument_group("SST", "Configure SST dataset.")
    group.add_argument(
        "--dataset",
        default="test",
    ...
    while not env.done:
        ...
        if finetune:
            if len(data_buffer) >= 5:
                context_np = data_buffer[0]  # SST.BENCHMARK.IN_LEN frames
                gt_np = np.concatenate(data_buffer[1:], axis=0)
                gt_np = gt_np[: cfg.SST.BENCHMARK.OUT_LEN]
                ...
                weighted_mse = (weights * np.square(stored_prediction[0] - gt_np)).sum(
                    axis=(2, 3, 4)
                )
        ...
        if mode == "online":
            ...
            context_np = np.repeat(context_np, cfg.MODEL.TRAIN.BATCH_SIZE, axis=1)
            orig_size = 1
            ...
            (
                ...
                begin_new_episode,
                need_upload_prediction,
            ) = env.get_observation(batch_size=cfg.MODEL.TRAIN.BATCH_SIZE)
            context_nd = mx.nd.array(context_np, ctx=ctx)
            context_nd = mx.nd.transpose(context_nd, axes=(1, 2, 0, 3, 4))
...
def train(args):
    ...
    (
        generator_net,
        loss_net,
    ) = construct_modules(args)

    # Prepare data
    train_hko_iter = SSTIterator(
        pd_path=cfg.SST_PD.RAINY_TRAIN,
        sample_mode="random",
        seq_len=cfg.SST.BENCHMARK.IN_LEN + cfg.SST.BENCHMARK.OUT_LEN,
    )
    ...
    for i in range(start_iter, cfg.MODEL.TRAIN.MAX_ITER):
        ...
        context_nd = (
            mx.nd.array(frame_dat[0 : cfg.SST.BENCHMARK.IN_LEN, ...], ctx=args.ctx[0])
            / 255.0
        )
        gt_nd = (
            mx.nd.array(
                frame_dat[
                    cfg.SST.BENCHMARK.IN_LEN : (
                        cfg.SST.BENCHMARK.IN_LEN + cfg.SST.BENCHMARK.OUT_LEN
                    )
                ],
                ctx=args.ctx[0],
            )
        ...
        # Train a step
        pred_nd, avg_l2, avg_real_mse, generator_grad_norm = train_step(
            generator_net, loss_net, context_nd, gt_nd, mask_nd
        )
