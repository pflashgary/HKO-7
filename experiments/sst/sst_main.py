...
from nowcasting.sst_evaluation import *
from nowcasting.utils import parse_ctx, logging_config
from nowcasting.sst_iterator import SSTIterator, precompute_mask
import numpy as np
...
def frame_skip_reshape(dat, frame_skip):
    ...
    ret = (
        dat.reshape((seq_len // frame_skip, frame_skip, B, -1))
        .transpose((0, 2, 1, 3))
...
def frame_skip_reshape_back(dat, frame_skip):
    ...
    ret = (
        dat.reshape((seq_len, B // frame_skip, frame_skip, -1))
        .transpose((0, 2, 1, 3))
        .reshape((seq_len * frame_skip, B // frame_skip, C, H, W))
    )
    return ret
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train the SST nowcasting model")
    parser.add_argument(
        "--batch_size",
        dest="batch_size",
    ...
    parser.add_argument(
        "--save_dir", help="The saving directory", required=True, type=str
    )
    parser.add_argument(
        "--ctx",
        dest="ctx",
    ...
    parser.add_argument(
        "--lr", dest="lr", help="learning rate", default=None, type=float
    )
    parser.add_argument(
        "--wd", dest="wd", help="weight decay", default=None, type=float
    )
    parser.add_argument(
        "--grad_clip",
        dest="grad_clip",
...
def run_benchmark(
    ...
    """Run the SST7 Benchmark given the training sequences
    ...
    """
    logging.info(
        "Begin Evaluation, mode=%s, finetune=%s, sample_num=%d,"
        " results will be saved to %s" % (mode, str(finetune), sample_num, save_dir)
    )
    if finetune:
        logging.info(str(cfg.MODEL.TEST.ONLINE))
    env = SSTBenchmarkEnv(pd_path=pd_path, save_dir=save_dir, mode=mode)
    states = EncoderForecasterStates(factory=sst_factory, ctx=context)
    ...
    while not env.done:
        if finetune:
            if len(stored_data) >= 5:
                ...
                init_states = EncoderForecasterStates(factory=sst_factory, ctx=context)
                if not cfg.MODEL.TEST.DISABLE_TBPTT:
                    init_states.update(
                        states_nd=[
                ...
                weights = get_balancing_weights_numba(
                    data=data_gt,
                    mask=gt_mask,
                    base_balancing_weights=cfg.SST.EVALUATION.BALANCING_WEIGHTS,
                    thresholds=env._all_eval._thresholds,
                )
                weighted_mse = (
                    weights * np.square(stored_prediction[0] - data_gt)
                ).sum(axis=(2, 3, 4))
        ...
        encoder_net.forward(
            ...
            data_batch=mx.io.DataBatch(
                data=[in_frame_nd] + states.get_encoder_states()
            ),
        )
...
def train(args):
    ...
    if cfg.MODEL.TRAIN.TBPTT:
        ...
        train_sst_iters = []
        train_sst_iter_restart = []
        for _ in range(cfg.MODEL.TRAIN.BATCH_SIZE):
            ele_iter = SSTIterator(
                pd_path=cfg.SST_PD.RAINY_TRAIN,
                sample_mode="sequent",
                seq_len=cfg.MODEL.IN_LEN + cfg.MODEL.OUT_LEN,
                stride=cfg.MODEL.IN_LEN,
            )
            ele_iter.random_reset()
            train_sst_iter_restart.append(True)
            train_sst_iters.append(ele_iter)
    else:
    ...
    sst_nowcasting = SSTNowcastingFactory(
    ...
    sst_nowcasting_online = SSTNowcastingFactory(
        batch_size=1, in_seq_len=cfg.MODEL.IN_LEN, out_seq_len=cfg.MODEL.OUT_LEN
    )
    encoder_net, forecaster_net, loss_net = encoder_forecaster_build_networks(
        factory=sst_nowcasting, context=args.ctx
    )
    t_encoder_net, t_forecaster_net, t_loss_net = encoder_forecaster_build_networks(
        factory=sst_nowcasting_online,
        context=args.ctx[0],
    ...
    states = EncoderForecasterStates(factory=sst_nowcasting, ctx=args.ctx[0])
    for info in sst_nowcasting.init_encoder_state_info:
        assert info["__layout__"].find("N") == 0, (
            "Layout=%s is not supported!" % info["__layout__"]
        )
    for info in sst_nowcasting.init_forecaster_state_info:
        assert info["__layout__"].find("N") == 0, (
            "Layout=%s is not supported!" % info["__layout__"]
        )
    ...
    while iter_id < cfg.MODEL.TRAIN.MAX_ITER:
        if not cfg.MODEL.TRAIN.TBPTT:
            ...
            frame_dat, mask_dat, datetime_clips, _ = train_sst_iter.sample(
            ...
            frame_dat_l = []
            mask_dat_l = []
            for i, ele_iter in enumerate(train_sst_iters):
                if ele_iter.use_up:
                    states.reset_batch(batch_id=i)
                    ele_iter.random_reset()
                    train_sst_iter_restart[i] = True
                if train_sst_iter_restart[i] == False and ele_iter.check_new_start():
                    states.reset_batch(batch_id=i)
                    ele_iter.random_reset()
                frame_dat, mask_dat, datetime_clips, _ = ele_iter.sample(
                    batch_size=cfg.MODEL.TRAIN.BATCH_SIZE
                )
                train_sst_iter_restart[i] = False
        ...
        data_nd = (
            mx.nd.array(frame_dat[0 : cfg.MODEL.IN_LEN, ...], ctx=args.ctx[0]) / 255.0
        )
        target_nd = (
            mx.nd.array(
                frame_dat[
                    cfg.MODEL.IN_LEN : (cfg.MODEL.IN_LEN + cfg.MODEL.OUT_LEN), ...
                ],
                ctx=args.ctx[0],
            )
            / 255.0
        ...
        if (iter_id + 1) % cfg.MODEL.SAVE_ITER == 0:
            encoder_net.save_checkpoint(
                prefix=os.path.join(base_dir, "encoder_net"), epoch=iter_id
            )
            forecaster_net.save_checkpoint(
                prefix=os.path.join(base_dir, "forecaster_net"), epoch=iter_id
            )
        iter_id += 1
