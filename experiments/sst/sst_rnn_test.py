...
from sst_main import run_benchmark
import os
import argparse
import logging
...
def parse_args():
    parser = argparse.ArgumentParser(description="Test the SST nowcasting model")
    parser.add_argument(
        "--cfg", dest="cfg_file", help="Optional configuration file", type=str
    )
    parser.add_argument(
        "--load_dir", help="The directory to load the model", default=None, type=str
    )
    parser.add_argument(
        "--load_iter", help="The iterator to load", default=-1, type=int
    )
    parser.add_argument(
