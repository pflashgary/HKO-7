...
from nowcasting.helpers.visualization import save_movie
from nowcasting.sst_iterator import SSTIterator
from nowcasting.sst_evaluation import SSTEvaluation


class SSTBenchmarkEnv(object):
    """The Benchmark environment for the SST7 Dataset
    ...
    def __init__(self, pd_path, save_dir="sst7_benchmark", mode="fixed"):
        ...
        self._mode = mode
        self._out_seq_len = cfg.SST.BENCHMARK.OUT_LEN
        self._stride = cfg.SST.BENCHMARK.STRIDE
        if mode == "fixed":
            self._in_seq_len = cfg.SST.BENCHMARK.IN_LEN
        else:
            self._in_seq_len = cfg.SST.BENCHMARK.STRIDE
        self._sst_iter = SSTIterator(
            pd_path=pd_path,
            sample_mode="sequent",
        ...
        self._save_seq_inds = set(
            np.arange(1, cfg.SST.BENCHMARK.VISUALIZE_SEQ_NUM + 1)
            * (self._stat_dict["pred_seq_num"] // cfg.SST.BENCHMARK.VISUALIZE_SEQ_NUM)
        )
        self._all_eval = SSTEvaluation(seq_len=self._out_seq_len, use_central=False)
    ...
    def _fingerprint(self):
        ...
        if self._mode == "fixed":
            fingerprint = (
                pd_file_name
                + "_in"
                + str(self._in_seq_len)
                + "_out"
                + str(self._out_seq_len)
                + "_stride"
                + str(self._stride)
                + "_"
                + self._mode
    ...
    def _stat_filepath(self):
        ...
        return os.path.join(cfg.SST.BENCHMARK.STAT_PATH, filename)
    ...
    def _get_benchmark_stat(self):
        ...
        if os.path.exists(self._stat_filepath):
            ...
            while not self._sst_iter.use_up:
                if self._mode == "fixed":
                    datetime_clips, new_start = self._sst_iter.sample(
                        batch_size=1024, only_return_datetime=True
                    )
                    ...
                    datetime_clips, new_start = self._sst_iter.sample(
                        batch_size=1, only_return_datetime=True
                    )
                    if len(datetime_clips) == 0:
                        continue
                    episode_num += new_start
                    if new_start:
                        episode_start_datetime.append(
                            datetime_clips[0][0].strftime("%Y%m%d%H%M")
                        )
                        if self._stride != 1:
                            seq_num += 1
                    else:
    ...
    def get_observation(self, batch_size=1):
        ...
        assert not self._sst_iter.use_up
        while True:
            frame_dat, mask_dat, datetime_clips, new_start = self._sst_iter.sample(
                batch_size=batch_size, only_return_datetime=False
            )
            if len(datetime_clips) == 0:
                continue
            else:
        ...
        self._out_datetime_clips = [
            ele[self._in_seq_len : (self._in_seq_len + self._out_seq_len)]
            for ele in datetime_clips
        ]
        self._in_frame_dat = frame_dat[: self._in_seq_len, ...]
        self._out_frame_dat = frame_dat[
            self._in_seq_len : (self._in_seq_len + self._out_seq_len), ...
        ]
        self._in_mask_dat = mask_dat[: self._in_seq_len, ...]
        self._out_mask_dat = mask_dat[
            self._in_seq_len : (self._in_seq_len + self._out_seq_len), ...
        ]
        return (
            self._in_frame_dat,
    ...
    def upload_prediction(self, prediction):
        ...
        """
        assert self._need_upload_prediction, (
            "Must call get_observation first!"
        ...
        if len(save_ind_set) > 0:
            ...
            print(
                "Saving prediction videos to %s"
                % os.path.join(self._save_dir, self._fingerprint)
            )
            min_data = min(
                np.min(
                    self._in_frame_dat[:, ind, 0, ...],
                ),
                np.min(self._out_frame_dat[:, ind, 0, ...]),
                np.ndarray.min(prediction[:, ind, 0, ...]),
            )
            max_data = max(
                np.max(
                    self._in_frame_dat[:, ind, 0, ...],
                ),
                np.max(self._out_frame_dat[:, ind, 0, ...]),
                np.ndarray.max(prediction[:, ind, 0, ...]),
            )
            save_movie(
    ...
    def print_stat_readable(self):
        self._all_eval.print_stat_readable(
            prefix="Received:%d " % self._received_pred_seq_num
        )

    def save_eval(self):
        assert (
            self._received_pred_seq_num == self._stat_dict["pred_seq_num"]
        ), "Must upload all the predictions to the testbed!"
        # print(
        #     "Saving evaluation result to %s"
        #     % os.path.join(self._save_dir, self._fingerprint)
        # )
        # self._all_eval.save(
        #     prefix=os.path.join(self._save_dir, self._fingerprint, "eval_all")
        # )


if __name__ == "__main__":
    env_fixed = SSTBenchmarkEnv(pd_path=cfg.SST_PD.RAINY_TEST, mode="fixed")
    env_online = SSTBenchmarkEnv(pd_path=cfg.SST_PD.RAINY_TEST, mode="online")
    ...
    env_fixed = SSTBenchmarkEnv(pd_path=cfg.SST_PD.RAINY_VALID, mode="fixed")
    env_online = SSTBenchmarkEnv(pd_path=cfg.SST_PD.RAINY_VALID, mode="online")
    ...
    env_fixed = SSTBenchmarkEnv(pd_path=cfg.SST_PD.ALL_15, mode="fixed")
    env_online = SSTBenchmarkEnv(pd_path=cfg.SST_PD.ALL_15, mode="online")
