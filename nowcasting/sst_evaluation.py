...
# from nowcasting.sst_iterator import get_exclude_mask
...
def pixel_to_rainfall(img, a=None, b=None):
    ...
    """
    if a is None:
        a = cfg.SST.EVALUATION.ZR.a
    if b is None:
        b = cfg.SST.EVALUATION.ZR.b
...
def get_hit_miss_counts(prediction, truth, mask=None, thresholds=None, sum_batch=False):
    ...
    if thresholds is None:
        thresholds = cfg.SST.EVALUATION.THRESHOLDS
    assert 5 == prediction.ndim
    assert 5 == truth.ndim
    assert prediction.shape == truth.shape
    assert prediction.shape[2] == 1
    thresholds = rainfall_to_pixel(
        np.array(thresholds, dtype=np.float32).reshape((1, 1, len(thresholds), 1, 1))
    )
    ...
    if mask is None:
        ...
        hits = np.logical_and(np.logical_and(bpred, btruth), mask).sum(
            axis=summation_axis
        )
        misses = np.logical_and(np.logical_and(bpred_n, btruth), mask).sum(
            axis=summation_axis
        )
        false_alarms = np.logical_and(np.logical_and(bpred, btruth_n), mask).sum(
            axis=summation_axis
        )
        correct_negatives = np.logical_and(np.logical_and(bpred_n, btruth_n), mask).sum(
            axis=summation_axis
        )
...
def get_correlation(prediction, truth):
    ...
    ret = (prediction * truth).sum(axis=(3, 4)) / (
        np.sqrt(np.square(prediction).sum(axis=(3, 4)))
        * np.sqrt(np.square(truth).sum(axis=(3, 4)))
        + eps
    )
...
def get_rainfall_mse(prediction, truth):
    ret = np.square(pixel_to_rainfall(prediction) - pixel_to_rainfall(truth)).mean(
        axis=(2, 3)
    )
    ret = ret.sum(axis=1)
    return ret
...
def get_SSIM(prediction, truth):
    ...
    truth = truth.reshape(
        (truth.shape[0] * truth.shape[1], truth.shape[3], truth.shape[4], 1)
    )
...
def get_balancing_weights(data, mask, base_balancing_weights=None, thresholds=None):
    ...
    thresholds = rainfall_to_pixel(
        np.array(thresholds, dtype=np.float32).reshape((1, 1, 1, 1, 1, len(thresholds)))
    )
    weights = np.ones_like(data) * base_balancing_weights[0]
    threshold_mask = np.expand_dims(data, axis=5) >= thresholds
    base_weights = np.diff(np.array(base_balancing_weights, dtype=np.float32)).reshape(
        (1, 1, 1, 1, 1, len(base_balancing_weights) - 1)
    )
...
try:
    from nowcasting.numba_accelerated import (
        get_GDL_numba,
...
class SSTEvaluation(object):
    def __init__(
        self, seq_len, use_central, no_ssim=True, threholds=None, central_region=None
    ):
        if central_region is None:
            central_region = cfg.SST.EVALUATION.CENTRAL_REGION
        self._thresholds = (
            cfg.SST.EVALUATION.THRESHOLDS if threholds is None else threholds
        )
    ...
    def print_stat_readable(self, prefix=""):
        logging.info(
            "%sTotal Sequence Number: %d, Use Central: %d"
        ...
        logging.info(
            ...
            + ", ".join(
                [
                    ">%g:%g/%g" % (threshold, pod[:, i].mean(), pod[-1, i])
                    for i, threshold in enumerate(self._thresholds)
                ]
            )
        )
        logging.info(
            ...
            + ", ".join(
                [
                    ">%g:%g/%g" % (threshold, far[:, i].mean(), far[-1, i])
                    for i, threshold in enumerate(self._thresholds)
                ]
            )
        )
        logging.info(
            ...
            + ", ".join(
                [
                    ">%g:%g/%g" % (threshold, csi[:, i].mean(), csi[-1, i])
                    for i, threshold in enumerate(self._thresholds)
                ]
            )
        )
        logging.info(
            ...
            + ", ".join(
                [
                    ">%g:%g/%g" % (threshold, gss[:, i].mean(), gss[-1, i])
                    for i, threshold in enumerate(self._thresholds)
                ]
            )
        )
        logging.info(
            ...
            + ", ".join(
                [
                    ">%g:%g/%g" % (threshold, hss[:, i].mean(), hss[-1, i])
                    for i, threshold in enumerate(self._thresholds)
                ]
            )
        )
    ...
    def save_pkl(self, path):
        ...
        f = open(path, "wb")
        logging.info("Saving SSTEvaluation to %s" % path)
    ...
    def save_txt_readable(self, path):
        ...
        # pod, far, csi, hss, gss, mse, mae, gdl = self.calculate_stat()
        f = open(path, "w")
        logging.info("Saving readable txt of SSTEvaluation to %s" % path)
        f.write(
            "Total Sequence Num: %d, Out Seq Len: %d, Use Central: %d\n"
            % (self._total_batch_num, self._seq_len, self._use_central)
        )
        for (i, threshold) in enumerate(self._thresholds):
            f.write("Threshold = %g:\n" % threshold)
            f.write("   POD: %s\n" % str(list(pod[:, i])))
        ...
        f.write(
            "Balanced MSE stat: avg %g/final %g\n"
            % (balanced_mse.mean(), balanced_mse[-1])
        )
        f.write(
            "Balanced MAE stat: avg %g/final %g\n"
