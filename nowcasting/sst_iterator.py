...
import numpy as np
import bisect
from nowcasting import sst_nc
from nowcasting.mask_nc import *
...
def get_valid_datetime_set():
    valid_datetime_set = pickle.load(open(cfg.SST_VALID_DATETIME_PATH, "rb"))
    return valid_datetime_set


def get_exclude_mask():
    with np.load(os.path.join(cfg.SST_DATA_BASE_PATH, "mask_dat.npz")) as dat:
...
def convert_datetime_to_filepath(date_time):
    ...
    ret = os.path.join(
        ...
        "%02d" % date_time.month,
        "%02d" % date_time.day,
        "%04d%02d%02d090000-JPL-L4_GHRSST-SSTfnd-MUR-GLOB-v02.0-fv04.1_crop.nc"
        % (date_time.year, date_time.month, date_time.day),
    )
    ret = os.path.join(cfg.SST_NC_PATH, ret)
    return ret
...
def convert_datetime_to_maskpath(date_time):
    ...
    ret = os.path.join(cfg.SST_MASK_PATH, ret)
    return ret


class SSTSimpleBuffer(object):
    ...
    def _load(self):
        ...
        self._frame_dat = sst_nc.quick_read_frames(
            path_list=paths, im_h=self._height, im_w=self._width
        )
        self._frame_dat = self._frame_dat.reshape(
            (self._max_buffer_length, 1, self._height, self._width)
        )
        self._noise_mask_dat = np.zeros(
            (self._datetime_keys.size, 1, self._height, self._width), dtype=np.uint8
        )
    ...
    def get(self, timestamps):
        ...
        """
        if not (
            timestamps[0] in self._datetime_keys
            and timestamps[-1] in self._datetime_keys
        ):
        ...
        return (
            self._frame_dat[begin_ind:end_ind, :, :, :],
            self._noise_mask_dat[begin_ind:end_ind, :, :, :],
        )


def pad_sst_dat(frame_dat, mask_dat, batch_size):
    if frame_dat.shape[1] < batch_size:
        ret_frame_dat = np.zeros(
            shape=(
                frame_dat.shape[0],
                batch_size,
                frame_dat.shape[2],
...
def precompute_mask(img):
    if img.dtype == np.uint8:
        threshold = round(cfg.SST.ITERATOR.FILTER_RAINFALL_THRESHOLD * 255.0)
    else:
        threshold = cfg.SST.ITERATOR.FILTER_RAINFALL_THRESHOLD
    mask = np.zeros_like(img, dtype=bool)
    mask[:] = np.broadcast_to((1 - _exclude_mask).astype(bool), shape=img.shape)
    mask[np.logical_and(img < threshold, img > 0)] = 0
    return mask


class SSTIterator(object):
    """The iterator for SST dataset"""
    ...
    def __init__(
        ...
        Sequent sample: sample a clip from the beginning of the time.
                        Everytime, the clips from {T_begin, T_begin + 1440min, ..., T_begin + (seq_len-1) * 1440min} will be used
                        The begin datetime will move forward by adding stride: T_begin += 1440min * stride
        ...
        """
        if width is None:
            width = cfg.SST.ITERATOR.WIDTH
        if height is None:
            height = cfg.SST.ITERATOR.HEIGHT
    ...
    def _is_valid_clip(self, datetime_clip):
        ...
        for i in range(len(datetime_clip)):
            if datetime_clip[i] not in self._df_index_set:
                missing_count += 1
                if (
                    missing_count > self._max_consecutive_missing
                    or missing_count >= len(datetime_clip)
                ):
                    return False
            else:
                missing_count = 0
    ...
    def _load_frames(self, datetime_clips):
        ...
        frame_dat = np.zeros(
            (self._seq_len, batch_size, 1, self._height, self._width), dtype=np.uint8
        )
        mask_dat = np.zeros(
            (self._seq_len, batch_size, 1, self._height, self._width), dtype=bool
        )
        if self.sample_mode == "random":
            paths = []
            mask_paths = []
            ...
            for i in range(self._seq_len):
                for j in range(batch_size):
                    ...
                    if timestamp in self._df_index_set:
                        ...
                        mask_paths.append(
                            convert_datetime_to_maskpath(datetime_clips[j][i])
                        )
                        hit_inds.append([i, j])
                    else:
                        miss_inds.append([i, j])
            hit_inds = np.array(hit_inds, dtype=int)
            all_frame_dat = sst_nc.quick_read_frames(
                path_list=paths, im_h=self._height, im_w=self._width
            )
            all_mask_dat = quick_read_masks(mask_paths, len(hit_inds[:, 0]))
            ...
            if self._buffer_datetime_keys is None or not (
                first_timestamp in self._buffer_datetime_keys
                ...
                for i in range(self._buffer_datetime_keys.size):
                    paths.append(
                        convert_datetime_to_filepath(self._buffer_datetime_keys[i])
                    )
                    mask_paths.append(
                        convert_datetime_to_maskpath(self._buffer_datetime_keys[i])
                    )
                self._buffer_frame_dat = sst_nc.quick_read_frames(
                    path_list=paths, im_h=self._height, im_w=self._width
                )
                self._buffer_mask_dat = quick_read_masks(
                    mask_paths, len(self._buffer_frame_dat)
                )
            for i in range(self._seq_len):
                for j in range(batch_size):
                    timestamp = datetime_clips[j][i]
    ...
    def random_reset(self):
        assert self.sample_mode == "sequent"
        self.set_begin_end(
            begin_ind=np.random.randint(0, self.total_frame_num - 5 * self._seq_len),
            end_ind=None,
        )
        self._current_datetime = self.begin_time
    ...
    def check_new_start(self):
        assert self.sample_mode == "sequent"
        datetime_clip = pd.date_range(
            start=self._current_datetime, periods=self._seq_len, freq=self._base_freq
        )
    ...
    def sample(self, batch_size, only_return_datetime=False):
        """Sample a minibatch from the sst dataset based on the given type and pd_file
        ...
        if self.sample_mode == "sequent":
            if self.use_up:
                raise ValueError("The SSTIterator has been used up!")
            datetime_clips = []
            new_start = False
            for i in range(batch_size):
                while not self.use_up:
                    datetime_clip = pd.date_range(
                        start=self._current_datetime,
                    ...
                    if self._is_valid_clip(datetime_clip):
                        ...
                        new_start = True
                        self._current_datetime = self._next_exist_timestamp(
                        ...
                        if self._current_datetime is None:
                            ...
                            self._current_datetime = (
                                self.end_time + self._base_time_delta
                            )
                        continue
...
# Simple test for the performance of the SST iterator.
if __name__ == "__main__":
    np.random.seed(123)
    import time
    import cProfile, pstats
    from nowcasting.config import cfg
    from nowcasting.helpers.visualization import save_gif, save_movie

    minibatch_size = 32
    seq_len = 30
    train_sst_iter = SSTIterator(
        pd_path=cfg.SST_PD.RAINY_TRAIN, sample_mode="random", seq_len=seq_len
    )
    valid_sst_iter = SSTIterator(
        pd_path=cfg.SST_PD.RAINY_VALID, sample_mode="sequent", seq_len=seq_len, stride=5
    )
    test_sst_iter = SSTIterator(
        pd_path=cfg.SST_PD.RAINY_TEST, sample_mode="sequent", seq_len=seq_len, stride=5
    )
    ...
    for i in range(repeat_time):
        (
            sample_sequence,
            sample_mask,
            sample_datetime_clips,
            new_start,
        ) = train_sst_iter.sample(batch_size=minibatch_size)
    end = time.time()
    pr.disable()
    ps = pstats.Stats(pr).sort_stats("cumulative")
    ps.print_stats(20)
    print(
        "Train Data Sample FPS: %f"
        % (minibatch_size * seq_len * repeat_time / float(end - begin))
    )
    ...
    for i in range(repeat_time):
        (
            ...
            sample_datetimes,
            new_start,
        ) = valid_sst_iter.sample(batch_size=minibatch_size)
    end = time.time()
    print(
        "Valid Data Sample FPS: %f"
        % (minibatch_size * seq_len * repeat_time / float(end - begin))
    )
    begin = time.time()
    for i in range(repeat_time):
        (
            ...
            sample_datetimes,
            new_start,
        ) = test_sst_iter.sample(batch_size=minibatch_size)
    end = time.time()
    print(
        "Test Data Sample FPS: %f"
        % (minibatch_size * seq_len * repeat_time / float(end - begin))
    )
    ...
    train_time = 0
    for i in range(30):
        train_batch, train_mask, sample_datetimes, new_start = train_sst_iter.sample(
            batch_size=minibatch_size
        )
        name_str = (
            "train_" + str(i) + "_" + sample_datetimes[0][0].strftime("%Y%m%d%H%M")
        )
        save_movie(
            train_batch[:, 0, 0, :, :],
            sample_datetimes[0],
    ...
    valid_time = 0
    while not valid_sst_iter.use_up:
        valid_batch, valid_mask, sample_datetimes, new_start = valid_sst_iter.sample(
            batch_size=minibatch_size
        )
        ...
        name_str = (
            ...
            + str(valid_time)
            + "_"
            + sample_datetimes[0][0].strftime("%Y%m%d%H%M")
        )
        save_movie(
            valid_batch[:, 0, 0, :, :],
            sample_datetimes[0],
        ...
        toc = time.time()
        save_movie(
            valid_mask[:, 0, 0, :, :].astype(np.uint8) * 255,
            sample_datetimes[0],
            None,
            masked=False,
            save_path=name_str + "_mask.mp4",
