...
class EC2InstanceStack(core.Stack):
    ...
    def __init__(self, scope: core.Construct, id: str, **kwargs) -> None:
        ...
        # Fetch VPC info
        vpc = ec2.Vpc.from_lookup(self, "VPC", vpc_id=self.config["vpc_id"][self.region])
        ...
        ec2_sg = ec2.SecurityGroup(
            self, "EC2SG", description=f'{self.config["prefix"]} SG (for EC2)', vpc=vpc,
        )
                ...
                "extra-permissions": iam.PolicyDocument(
                    statements=[
                        iam.PolicyStatement(
                            actions=["s3:*"],
                            resources=[
                                f"arn:aws:iam::aws:policy/AmazonS3FullAccess",
                                f"arn:aws:s3:::metservice-research-{self.region}/research/archive-data/ec2dt/*"
                            ],
                        )
                    ]
                )
